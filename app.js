...
app.post("/addbook", function (req, res) {
    ...
    [Math.floor(Number(req.body.point_value) + 3 / 3), req.body.book_owner],
    function(err, result){
        if(err){
            console.log(err)
            next(err)
...
app.post("/addswap", function (req, res) {
    ...
    mysql.pool.query("INSERT INTO Swaps(request_user, owning_user, book, swap_status) VALUES (\
        (SELECT userid FROM Users WHERE username=?),(SELECT userid FROM Users WHERE username=?),?,1)",
    [req.body.reqUser, req.body.owner, Number(req.body.bookid)],
    function(err, result){
        if(err){
            console.log(err)
            next(err)
...
app.post("/rejectswap", function (req, res) {
    ...
    mysql.pool.query("DELETE FROM Swaps WHERE owning_user=(SELECT userid FROM users WHERE username=?) AND book=(SELECT bookid FROM Books WHERE title=?)",
